...
function renderMap() {
  ...
  rows.forEach((row, index) => {
    ...
    rowDiv.className = `row row-${index + 1}`;
    row.forEach(hexId => {
...
function createHexElement(hexId) {
  ...
  const size = Math.min(150 * zoomLevel, 300);
  hex.style.width = size + 'px';
  hex.style.height = (size * 0.86) + 'px';
...
document.querySelector('.close')?.addEventListener('click',()=>{document.getElementById('info-modal').style.display='none';});
